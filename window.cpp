...
void Camera::update(double dt){
	...
	if (this->enabled==true){
		this->_drx+=(this->_w->y+this->_w->h/2.0-mp.y)*0.075;
		this->_dry+=(this->_w->x+this->_w->w/2.0-mp.x)*0.075;
		if (this->_drx>90){
			this->_drx=90;
		}
		if (this->_drx<-90){
		...
		float dx=sin(-(this->_drx-90)*4*atan(1)/180.0)*CAMERA_MOVE_SPEED;
		float dz=cos(-this->_dry*4*atan(1)/180.0)*CAMERA_MOVE_SPEED;
		if (this->_w->pressed(87)){
			this->_dx+=dx;
			this->_dz-=dz;
		}
		this->x=this->_dx+0;///////////////
		this->y=this->_dy+0;///////////////
		this->z=this->_dz+0;///////////////
		this->rx=(this->_drx-90)*4*atan(1)/180.0;///////////////
		this->ry=this->_dry*4*atan(1)/180.0;///////////////
	}
	if (this->lock==true){
		SetCursorPos(this->_w->x+this->_w->w/2,this->_w->y+this->_w->h/2);
	}
	this->matrix=DirectX::XMMatrixLookToLH(DirectX::XMVectorSet(this->x,this->y,this->z,0),DirectX::XMVectorSet(sin(this->rx)*cos(this->ry),cos(this->rx),sin(this->rx)*sin(this->ry),0),DirectX::XMVectorSet(0,1,0,0));
}
...
void Renderer::_r(bool s){
	...
	this->view_matrix=DirectX::XMMatrixIdentity();
...
Matrix RendererHelper::create_lookat_matrix(Vector e,Vector f,Vector u){
		...
		if (PeekMessage(&msg,this->_hwnd,0,0,PM_REMOVE)==1){
			...
			switch (msg.message){
				...
				case WM_MOUSEMOVE:
					{
						MSG p=msg;
						this->mx=GET_X_LPARAM(p.lParam);
						this->my=GET_Y_LPARAM(p.lParam);
					}
					break;
				default:
					DispatchMessage(&msg);
