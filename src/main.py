...
MINECRAFT_JAVA_RUNTIME_FILE_PATH="C:/Program Files/Java/jdk-17.0.1/bin/java.exe"
...
MINECRAFT_SKIP_UPDATE=["21w37a","21w38a","22w13oneblockatatime"]
...
def _push_single_project(p,b_nm):
	...
	if (any([(True if b[1] is not None else False) for b in bl]) and (cnt[0]>0 or cnt[3]>0)):
		...
		tm_s=time.strftime(f"%Y-%m-%dT%H:%M:%S{('-' if UTC_OFFSET<0 else '+')}{abs(UTC_OFFSET)//3600:-02d}:{abs(UTC_OFFSET//60)%60:02d}",time.gmtime(c_tm+UTC_OFFSET))
